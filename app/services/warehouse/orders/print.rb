  ...
  module Orders
    class Print < BaseService
      ...
      def generate_report
        ...
        command = "php #{report} #{Rails.env} #{@order_id} '#{@order_params['consumer_fio'] || @order_params['consumer_tn']}' '#{l_date}' '#{invent_item_params.to_json}' '#{warehouse_item_params.to_json}'"
